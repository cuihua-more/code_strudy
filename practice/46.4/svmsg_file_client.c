...
int main(char argc, char *argv[])
{
    ...
    serverIdFd = open(SERVER_KEY_FILE, O_RDONLY);
    if (serverIdFd == -1) {
    ...
    if (read(serverIdFd, (void *)&serverId, sizeof(serverId)) != sizeof(serverId)) {
        fprintf(stderr, "read server System V IPC Message Queue ID file failed! Path is %s, line = %d\n",
                SERVER_KEY_FILE, __LINE__);
        close(serverIdFd);
        exit(EXIT_FAILURE);
    }
